...
long long calculateEnergy(int posLeft, int posMiddle, int posRight, const vector<int>& stabilityPotentials, 
    ...
    //Calculate energy
    long long E1 = P_left*affinity[BC_left][BC_middle]*P_middle;
    long long E2 = P_middle*affinity[BC_middle][BC_right]*P_right;
...
long long calculateTotalEnergy(int n, int l, int r, int k, const vector<vector<long long>>& maxEnergy,  
                            const vector<int> stabilityPotentials, const vector<int> biochemicalClasses) {
    long long leftEnergy = (k > l) ? maxEnergy[l][k-1] : 0;
    long long rightEnergy = (k < l) ? maxEnergy[k+1][r] : 0;

    //Calculate energy when removing k last
    int posMiddle = k;
    int posLeft = (l == 0) ? -1 : l - 1;
    int posRight = (r == n - 1) ? n : r + 1;
    long long kEnergy = calculateEnergy(posLeft, posMiddle, posRight, stabilityPotentials, biochemicalClasses);
...
void fillMaxEnergyTable(int n, vector<vector<long long>>& maxEnergy, vector<vector<int>>& lastRemoved, 
                            const vector<int> stabilityPotentials, const vector<int> biochemicalClasses){
    //Base cases (intervals of size 1)
    for(int i = 0; i < n; i++) {
        int posMiddle = i;
        int posLeft = (i == 0) ? -1 : i - 1;
        int posRight = (i == n - 1) ? n : i + 1;
    ...
    for(int intervalSize = 2; intervalSize <= n; intervalSize++) {
        for(int l = 0; l <= n-intervalSize; l++) {
            ...
            int k = l;
            long long bestEnergy = calculateTotalEnergy(n, l, r, k, maxEnergy, stabilityPotentials, biochemicalClasses);
            int bestK = k;
        }
        //FIX ME -- for cm o k
    }
...
int main() {
    ...
    vector<vector<long long>> maxEnergy(n, vector<long long>(n, 0));
